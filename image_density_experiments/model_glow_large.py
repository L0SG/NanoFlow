...
class Block(nn.Module):
    ...
    def forward(self, flow_embed, input):
        out = input
...
class FlowStep(nn.Module):
    ...
    def forward(self, estimator, flow_embed, i_flow, input, logdet=None, reverse=False):
        if not reverse:
            return self.normal_flow(estimator, flow_embed, i_flow, input, logdet)
        else:
            return self.reverse_flow(estimator, flow_embed, i_flow, input, logdet)
    ...
    def reverse_flow(self, estimator, flow_embed, i_flow, input, logdet):
        assert input.size(1) % 2 == 0
...
class FlowNet(nn.Module):
    ...
    def encode(self, z, logdet=0.0):
        for layer, shape, i_flow in zip(self.layers, self.output_shapes, self.flow_idx):
            if isinstance(layer, SqueezeLayer) or isinstance(layer, Split2d):
                z, logdet = layer(z, logdet, reverse=False)
            elif isinstance(layer, FlowStep):
                flow_embed_i = None
                z, logdet = layer(self.estimator[i_flow], flow_embed_i, i_flow, z, logdet, reverse=False)

        return z, logdet

    def decode(self, z, temperature=None):
        for layer, i_flow in zip(reversed(self.layers), reversed(self.flow_idx)):
            if isinstance(layer, Split2d):
                ...
                flow_embed_i = None
                z, logdet = layer(self.estimator[i_flow], flow_embed_i, i_flow, z, logdet=0, reverse=True)
        return z


class GlowLarge(nn.Module):
