...
class Block(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_channels):
        ...
        self.conv2 = Conv2d(hidden_channels, hidden_channels, kernel_size=(1,1))
...
class FlowStep(nn.Module):
    def __init__(self, in_channels, hidden_channels, actnorm_scale,
        ...
        if flow_coupling == "additive":
            self.conv_proj = Conv2dZeros(hidden_channels, in_channels // 2, kernel_size=(3,3))
        elif flow_coupling == "affine":
            self.conv_proj = Conv2dZeros(hidden_channels, in_channels, kernel_size=(3,3))
    ...
    def forward(self, estimator, flow_embed, i_flow, input, logdet=None, reverse=False):
        if not reverse:
            ...
            return self.reverse_flow(estimator, flow_embed, input, logdet)
...
class NanoFlowAltDecomp(nn.Module):
