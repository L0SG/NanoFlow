...
class WaveFlowCoupling2D(nn.Module):
    def __init__(self, in_channel, cin_channel,
                 ...
                 coupling_type='affine', num_bin=32, tail_bound=5.0):
        super().__init__()
...
class Flow(nn.Module):
    def __init__(self, in_channel, cin_channel, n_flow, filter_size, num_layer, num_height, layers_per_dilation_h_cycle,
                 coupling_type, num_bin, tail_bound, bipartize=False):
        super().__init__()
    ...
    def forward(self, x, c=None, i=None, debug=False):
        ...
        if i < int(self.n_flow / 2):
            ...
            if self.bipartize:
                ...
                out = bipartize_reverse_order(x)
                c = bipartize_reverse_order(c)
            else:
                out = reverse_order(x)
                c = reverse_order(c)

        out, logdet_couple, log_s, t = self.coupling(out, c, debug)
        logdet = logdet + logdet_couple
...
class WaveFlow(nn.Module):
    def __init__(self, in_channel, cin_channel, res_channel, n_height, n_flow, n_layer, layers_per_dilation_h_cycle,
                 coupling_type, num_bin=32, tail_bound=5.0, bipartize=False):
        super().__init__()
        self.in_channel = in_channel
        ...
        for i in range(self.n_flow):
            self.flows.append(Flow(self.in_channel, self.cin_channel, n_flow=self.n_flow, filter_size=self.res_channel,
                                   ...
                                   num_bin=num_bin, tail_bound=tail_bound, bipartize=self.bipartize))
    ...
    def reverse_fast(self, c, temp=1.0, debug_z=None):
        # optimized reverse without redundant computations
        c = self.upsample(c)
        # trim conv artifacts. maybe pad spec to kernel multiple
        time_cutoff = self.upsample_conv_kernel_size - self.upsample_conv_stride
        c = c[:, :, :-time_cutoff]
...
if __name__ == "__main__":
    ...
    net = WaveFlow(1, 80, 64, 8, 8, 4, 1, coupling_type='nsf', num_bin=7, tail_bound=11.0, bipartize=False).cuda()
    out = net(x, c)
